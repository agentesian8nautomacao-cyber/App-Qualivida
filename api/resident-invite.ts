/**
 * API: GET /api/resident-invite?token=xxx
 * Retorna dados do convite (email, expiresAt) se o token for válido e não expirado/não usado.
 */
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (error) {
        console.error('[resident-invite]', error);
        return Response.json(
      ...
      return Response.json(
        { email: data.email, expiresAt: data.expires_at },
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error('[resident-invite]', msg);
      return Response.json(
