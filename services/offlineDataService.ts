...
export { upsertCachedRecord, setCachedTable };
...
export async function createData(
  ...
  try {
    // occurrences: não enviar updated_at se a coluna não existir no projeto (migration 010)
    const insertPayload =
      table === 'occurrences'
        ? (() => {
    ...
    if (error) {
      ...
      await queueOutbox(table, 'INSERT', localPayload);
      // Para occurrences: devolver o erro ao app para o usuário ver e para debug (ex.: RLS, coluna)
      if (table === 'occurrences') {
        const errMsg = error.message || (error as any).error_description || String(error);
        return { success: false, id: localId, error: errMsg };
      }
      return { success: true, id: localId, queued: true };
    }
    ...
    await queueOutbox(table, 'INSERT', localPayload);
    if (table === 'occurrences') {
      return { success: false, id: localId, error: err?.message || String(err) };
    }
    return { success: true, id: localId, queued: true };
  }
}
...
async function processOutboxEntry(entry: OutboxRecord): Promise<void> {
  ...
  if (entry.operation === 'INSERT') {
    // Tabela occurrences pode não ter coluna updated_at (migration 010); remover para o sync não falhar.
    const insertPayload = table === 'occurrences' ? (() => { const p = { ...payload }; delete p.updated_at; return p; })() : payload;
    const { data, error } = await supabase
      .from(table)
      .insert(insertPayload as any)
      .select()
