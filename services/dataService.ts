...
// ============================================
// CONVITES PARA MORADORES (link único por e-mail)
// ============================================

/**
 * Cria um convite para Morador. Retorna o link único.
 */
  ...
  try {
    ...
    const origin =
      (typeof window !== 'undefined' && window.location.origin)
        ...
        : (typeof (import.meta as any)?.env?.VITE_APP_URL === 'string' ? String((import.meta as any).env.VITE_APP_URL).replace(/\/$/, '') : '');
    if (!origin || !/^https?:\/\//.test(origin)) {
      return { success: false, error: 'URL do app não configurada.' };
    }
    const inviteLink = `${origin}/accept-resident-invite?token=${encodeURIComponent(token)}`;
    return { success: true, inviteLink };
  } catch (err: any) {
    console.error('[createResidentInvite]', err);
    return { success: false, error: err?.message ?? 'Erro ao criar convite.' };
  }
};

/**
 * Cria convites para vários e-mails (envio em massa). Retorna lista de { email, inviteLink } ou erro por e-mail.
 */
export const createResidentInvitesBulk = async (
  emails: string[],
  createdBy?: string,
  expiresInDays: number = 7
): Promise<{ success: boolean; invites?: { email: string; inviteLink: string }[]; errors?: { email: string; error: string }[] }> => {
  const emailSet = new Set<string>();
  const normalized: string[] = [];
  for (const e of emails) {
    const n = e.trim().toLowerCase();
    if (!n || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n)) continue;
    if (emailSet.has(n)) continue;
    emailSet.add(n);
    normalized.push(n);
  }
  if (normalized.length === 0) {
    return { success: false, errors: [{ email: '', error: 'Nenhum e-mail válido.' }] };
  }
  const origin =
    (typeof window !== 'undefined' && window.location.origin)
      ? window.location.origin
      : (typeof (import.meta as any)?.env?.VITE_APP_URL === 'string' ? String((import.meta as any).env.VITE_APP_URL).replace(/\/$/, '') : '');
  if (!origin || !/^https?:\/\//.test(origin)) {
    return { success: false, errors: [{ email: '', error: 'URL do app não configurada.' }] };
  }
  const invites: { email: string; inviteLink: string }[] = [];
  const errors: { email: string; error: string }[] = [];
  const expiresAt = new Date();
  expiresAt.setDate(expiresAt.getDate() + expiresInDays);
  for (const emailNorm of normalized) {
    const token = generateInviteToken();
    const { error } = await supabase.from('resident_invites').insert({
      email: emailNorm,
      token,
    ...
    if (error) {
      const msg = String(error.message || '').toLowerCase();
      errors.push({ email: emailNorm, error: msg.includes('duplicate') || msg.includes('unique') ? 'Convite já existe para este e-mail.' : error.message });
    } else {
      invites.push({ email: emailNorm, inviteLink: `${origin}/accept-resident-invite?token=${encodeURIComponent(token)}` });
    }
  }
  return { success: invites.length > 0, invites: invites.length ? invites : undefined, errors: errors.length ? errors : undefined };
};
