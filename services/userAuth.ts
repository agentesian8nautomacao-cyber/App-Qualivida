...
export const loginUser = async (
  ...
  // Permitir login por e-mail (ex.: usuários convidados que recebem "Faça login com seu e-mail e senha").
  // O usuário deve existir em auth.users com esse e-mail; a senha é validada pelo Supabase Auth.
  ...
  try {
    const bypassRoles = ['PORTEIRO','PORTARIA','SINDICO','SÍNDICO','DESENVOLVEDOR','DEVELOPER','DEV','ADMIN','ADMINISTRADOR','ADMINISTRADORA','CABO_TURMA'];
  ...
  try {
    // Resolver identificador -> email: username, email, nome completo, primeiro nome (users, staff, residents)
    // Senha é sempre a definida no cadastro (auth.users).
    let emailToUse = normalizedInput;
    if (!normalizedInput.includes('@')) {
      try {
        const usernameQ = normalizedInput;
        // 1) users: por username ou email
        const { data: userRow } = await supabase
          .from('users')
          .select('email')
          .or(`username.eq.${usernameQ},email.eq.${usernameQ}`)
          .maybeSingle();
        if (userRow?.email) {
        ...
        // 4) Usuário convidado: por nome completo ou primeiro nome (users, depois staff)
        if (!emailToUse.includes('@')) {
          const escaped = usernameQ.replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_');
          // Nome completo (exato, case-insensitive)
          const { data: userByName } = await supabase
            ...
            .maybeSingle();
          if (userByName?.email) {
            emailToUse = String(userByName.email).trim().toLowerCase();
          }
          if (!emailToUse.includes('@')) {
            const { data: staffByName } = await supabase
              ...
              .ilike('name', escaped)
              .maybeSingle();
            if (staffByName?.email) emailToUse = String(staffByName.email).trim().toLowerCase();
          }
          // Primeiro nome: só se houver um único match (evitar ambiguidade)
          if (!emailToUse.includes('@')) {
            const { data: usersByFirst } = await supabase
              .from('users')
              .select('email')
              .ilike('name', `${escaped}%`)
              .limit(2);
            if (usersByFirst?.length === 1 && usersByFirst[0]?.email) {
              emailToUse = String(usersByFirst[0].email).trim().toLowerCase();
            }
          }
          if (!emailToUse.includes('@')) {
            const { data: staffByFirst } = await supabase
              .from('staff')
              .select('email')
              .ilike('name', `${escaped}%`)
              .limit(2);
            if (staffByFirst?.length === 1 && staffByFirst[0]?.email) {
              emailToUse = String(staffByFirst[0].email).trim().toLowerCase();
            }
          }
        }
      } catch {
    ...
    if (!simpleEmailRegex.test(emailToUse)) {
      return {
        user: null,
        error: 'Credenciais inválidas. Verifique usuário e senha. Se você foi convidado por e-mail, use seu e-mail (e não o nome de usuário) para fazer login.'
      };
    }
...
export const updateUserProfile = async (
  ...
  try {
    ...
    const tableCandidates = ['users', 'staff', 'residents'];
    let row: any = null;
