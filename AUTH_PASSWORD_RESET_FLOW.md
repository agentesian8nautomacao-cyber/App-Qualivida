...
### 5.0. E-mail de convite (Portaria/ADM) vs recuperação de senha

| Fluxo | Quem envia | Onde configurar |
|-------|------------|-----------------|
| **Recuperação de senha** ("Esqueci minha senha") | **Supabase Auth** | Dashboard → Authentication → E-mails / SMTP. O app apenas chama `resetPasswordForEmail()`; o Supabase envia o e-mail. |
| **Convite por link** (Convidar Portaria/ADM) | **API do app** (`/api/send-invite-email`) | Variável de ambiente **RESEND_API_KEY** (e opcionalmente **RESEND_FROM**). O Supabase não envia e-mails customizados; esta API usa Resend. |

Se você já configurou **Resend** como SMTP no Supabase (ex.: `smtp.resend.com`), use a **mesma chave de API** do Resend na variável **RESEND_API_KEY** (Vercel ou `.env.local`) para que o convite seja enviado pelo mesmo provedor. Assim, recuperação de senha e convite usam o mesmo serviço de e-mail.
