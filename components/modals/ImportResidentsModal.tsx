...
import { extractTextFromPdf } from '../../services/pdfBoletoExtractor';

function parseCSVText(text: string, existingResidents: Resident[]): { residents: Resident[]; errors: string[] } {
  const lines = text.split(/\r?\n/).filter((line: string) => line.trim());
  if (lines.length < 2) {
    return { residents: [], errors: ['Arquivo deve ter pelo menos um cabeçalho e uma linha de dados (CSV ou tabela).'] };
  }
  const firstLine = lines[0];
  const separator = firstLine.includes('\t') && !firstLine.includes(',') ? '\t' : ',';
  const headers = lines[0].split(separator).map((h: string) => h.trim().toLowerCase());
  const requiredFields = ['nome', 'unidade', 'email'];
  const missingFields = requiredFields.filter((f: string) => !headers.some((h: string) => h.includes(f)));
  if (missingFields.length > 0) {
    return { residents: [], errors: [`Campos obrigatórios não encontrados: ${missingFields.join(', ')}`] };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header: string, index: number) => {
      ...
      if (headerLower.includes('nome') || headerLower.includes('name')) resident.name = value;
      else if (headerLower.includes('unidade') || headerLower.includes('unit') || headerLower.includes('apto') || headerLower.includes('apartamento')) resident.unit = value;
      else if (headerLower.includes('email') || headerLower.includes('e-mail')) resident.email = value;
      else if ((headerLower.includes('telefone') || headerLower.includes('phone') || headerLower.includes('tel')) && !headerLower.includes('whatsapp')) resident.phone = value;
      else if (headerLower.includes('whatsapp') || headerLower.includes('whats')) resident.whatsapp = value;
      else extraData[header] = value;
    });
    if (!resident.name || !resident.unit) {
      errors.push(`Linha ${i + 1}: Nome e Unidade são obrigatórios`);
      continue;
    }
    const emailVal = resident.email || '';
    if (!emailVal || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailVal)) {
      errors.push(`Linha ${i + 1}: E-mail válido obrigatório`);
      continue;
    }
    if (existingResidents.some((r: Resident) => r.unit === resident.unit)) {
      errors.push(`Linha ${i + 1}: Unidade ${resident.unit} já existe`);
      continue;
    }
    residents.push({
      id: resident.id!,
...
/** Processa arquivo (qualquer tipo; tenta CSV ou JSON pelo conteúdo). Retorna moradores + erros. */
export async function processImportFile(
  file: File,
  existingResidents: Resident[]
): Promise<{ residents: Resident[]; errors: string[] }> {
  const fileType = file.name.split('.').pop()?.toLowerCase() || '';

  const tryCSV = async (): Promise<{ residents: Resident[]; errors: string[] }> => {
    const text = await file.text();
    return parseCSVText(text, existingResidents);
  };

  if (fileType === 'csv') return await tryCSV();
  ...
  if (fileType === 'json') {
    ...
    try {
      ...
      return { residents: [], errors: ['Arquivo JSON inválido.'] };
    }
    if (!Array.isArray(data)) {
      return { residents: [], errors: ['JSON deve ser um array de objetos.'] };
    }
    const residents: Resident[] = [];
    const errors: string[] = [];
    data.forEach((item: any, index: number) => {
      ...
      if (!emailVal || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(emailVal))) {
        errors.push(`Item ${index + 1}: E-mail válido obrigatório`);
        return;
      }
      const unit = item.unidade || item.unit || item.apto;
      if (existingResidents.some((r: Resident) => r.unit === unit)) {
        errors.push(`Item ${index + 1}: Unidade ${unit} já existe`);
        return;
      }
      residents.push({
  ...
  if (fileType === 'xlsx' || fileType === 'xls') {
    return { residents: [], errors: ['Importação de Excel: converta para CSV ou use JSON.'] };
  }

  // PDF: extrair texto com pdfjs e depois tentar JSON ou CSV
  if (fileType === 'pdf') {
    try {
      const text = (await extractTextFromPdf(file)).trim();
      if (!text) {
        return { residents: [], errors: ['O PDF está vazio ou não foi possível extrair texto.'] };
      }
      ...
      return parseCSVText(normalized, existingResidents);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Erro ao ler PDF.';
      return { residents: [], errors: [`PDF: ${msg}`] };
    }
  }

  // DOCX / DOC: extrair texto com mammoth e depois tentar JSON ou CSV
  if (fileType === 'docx' || fileType === 'doc') {
    try {
      ...
      if (!text) {
        return { residents: [], errors: ['O documento Word está vazio ou não foi possível extrair texto.'] };
      }
      // Tabelas no Word costumam vir com tabulação; normalizar para vírgula para o parser CSV
      text = text.replace(/\t+/g, ',');
      try {
      ...
      const msg = err instanceof Error ? err.message : 'Erro ao ler documento Word.';
      return { residents: [], errors: [`DOCX: ${msg}`] };
    }
  }

  // PDF, TXT e qualquer outro: tenta ler como texto; binários (PDF) falham aqui e tratamos abaixo
  try {
    let text: string;
    try {
      text = await file.text();
    } catch {
      // Arquivo binário (ex.: PDF): não dá para usar file.text(); retornar mensagem
      const ext = fileType || 'arquivo';
      return { residents: [], errors: [`Para .${ext}: use CSV, JSON ou DOCX. PDF não é suportado para importação de moradores.`] };
    }
    try {
      const data = JSON.parse(text);
      if (Array.isArray(data)) {
    ...
    return { residents: [], errors: ['Não foi possível ler o arquivo como texto. Use CSV ou JSON.'] };
  }
}
