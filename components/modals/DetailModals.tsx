      ...
      <div className="relative w-full max-w-4xl bg-zinc-900 text-white rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-12 animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar border border-white/10">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 mb-6 sm:mb-8 md:mb-12 pb-4 sm:pb-6 md:pb-8 border-b border-white/10">
          <div className="flex items-center gap-3 sm:gap-4 md:gap-6 w-full sm:w-auto">
             <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-[24px] sm:rounded-[28px] md:rounded-[32px] bg-linear-to-br from-blue-600 to-purple-600 flex items-center justify-center text-2xl sm:text-3xl md:text-4xl font-black shadow-lg shrink-0 overflow-hidden">
                {residentAvatar ? (
                  <img
                    src={residentAvatar}
             ...
             <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
                  ...
                  {(onEdit != null || onDelete != null) && (
                    <div className="flex items-center gap-2">
                      {onEdit != null && (
                        <button onClick={onEdit} className="p-1.5 sm:p-2 bg-white/5 rounded-lg sm:rounded-xl hover:bg-white text-white hover:text-black transition-all shrink-0" title="Editar"><Edit2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
                      )}
                      {onDelete != null && (
                        <button onClick={onDelete} className="p-1.5 sm:p-2 bg-red-500/20 text-red-400 rounded-lg sm:rounded-xl hover:bg-red-500 hover:text-white transition-all shrink-0" title="Excluir morador"><Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 sm:gap-3 mt-2 sm:mt-4">
                   {resident.whatsapp && <button onClick={() => openWhatsApp(resident.whatsapp, undefined, (error) => toast.error(`Erro ao abrir WhatsApp: ${error}`))} className="p-1.5 sm:p-2 bg-green-500/20 text-green-400 rounded-lg sm:rounded-xl hover:bg-green-500 hover:text-white transition-all shrink-0" title="WhatsApp"><MessageCircle className="w-4 h-4 sm:w-5 sm:h-5" /></button>}
                   {resident.email ? (
                     <a href={`mailto:${resident.email}`} className="p-1.5 sm:p-2 bg-white/5 rounded-lg sm:rounded-xl hover:bg-white hover:text-black transition-all shrink-0 inline-flex" title="E-mail"><Mail className="w-4 h-4 sm:w-5 sm:h-5" /></a>
                   ) : (
          ...
          <button onClick={onClose} className="p-2 sm:p-3 md:p-4 bg-white/5 rounded-2xl sm:rounded-3xl hover:bg-white/20 transition-all shrink-0 self-end sm:self-auto"><X className="w-5 h-5 sm:w-6 sm:h-6"/></button>
        </header>
      ...
      <div className="relative w-full max-w-xl bg-white text-black rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-12 animate-in zoom-in duration-500 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <header className="flex justify-between items-start gap-4 mb-6 sm:mb-8 md:mb-10">
          <div className="flex items-center gap-3 sm:gap-4 md:gap-6 min-w-0 flex-1">
            <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl sm:rounded-2xl bg-zinc-50 flex items-center justify-center shadow-inner shrink-0"><PackageIcon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 opacity-40" /></div>
          ...
          <button onClick={onClose} className="p-2 sm:p-3 md:p-4 bg-zinc-50 rounded-2xl sm:rounded-3xl hover:bg-zinc-100 transition-all shrink-0"><X className="w-5 h-5 sm:w-6 sm:h-6"/></button>
        </header>
        <div className="space-y-4 sm:space-y-6 md:space-y-8">
           ...
           <section className="space-y-4">
              ...
              <div className="p-4 sm:p-6 md:p-8 bg-zinc-50 rounded-[32px] sm:rounded-[40px] border border-black/5 space-y-3 sm:space-y-4">
                 ...
                 {pkg.items && pkg.items.length > 0 ? (<div className="space-y-2 sm:space-y-3 pt-2"><span className="text-[9px] font-black uppercase opacity-20 block">Inventário</span>{pkg.items.map((it: any, idx: number) => (<div key={idx} className="flex items-start gap-2 sm:gap-3"><div className="w-1.5 h-1.5 rounded-full bg-black mt-1.5 shrink-0" /><div className="min-w-0"><p className="text-xs sm:text-sm font-bold uppercase wrap-break-word">{it.name}</p>{it.description && <p className="text-[9px] sm:text-[10px] opacity-40 wrap-break-word">{it.description}</p>}</div></div>))}</div>) : (<p className="text-xs opacity-20 italic">Nenhum item específico detalhado.</p>)}
              </div>
           </section>
      ...
      <div className="relative w-full max-w-lg bg-white text-black rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-12 animate-in zoom-in duration-300">
         <header className="flex justify-between items-start gap-3 sm:gap-4 mb-6 sm:mb-8">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
               <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-2xl sm:rounded-3xl bg-zinc-100 flex items-center justify-center text-xl sm:text-2xl md:text-3xl font-black shadow-inner shrink-0">{visitor.visitorNames?.substring(0, 1).toUpperCase()}</div>
            ...
            <button onClick={onClose} className="p-2 sm:p-3 bg-zinc-100 rounded-xl sm:rounded-2xl hover:bg-zinc-200 shrink-0"><X className="w-4 h-4 sm:w-5 sm:h-5"/></button>
         </header>
         <div className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
               <div className="p-4 sm:p-5 bg-zinc-50 rounded-[20px] sm:rounded-[24px] border border-black/5"><p className="text-[9px] font-black uppercase opacity-30 mb-1">Destino</p><p className="text-xs sm:text-sm font-bold uppercase wrap-break-word">{visitor.unit} - {visitor.residentName}</p></div>
...
export const OccurrenceDetailModal = ({ occurrence, onClose, onSave, onDelete, onResolve, canDelete, canResolve, setOccurrence, currentRole, currentResident, currentAdminUser, onPersistMessage }: any) => {
  ...
  const handleSendMessage = () => {
    ...
    // Persistir no backend para o morador ver; notificação no sino é feita no App.
    if (onPersistMessage && ['SINDICO', 'PORTEIRO', 'ADMIN', 'ADMINISTRADOR', 'ADMINISTRADORA', 'CABO_TURMA'].includes(currentRole)) {
      onPersistMessage(updatedOccurrence);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-600 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white text-black rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-12 animate-in zoom-in duration-300 max-h-[90vh] overflow-hidden flex flex-col">
         <header className="flex justify-between items-start gap-3 sm:gap-4 mb-6 sm:mb-8 shrink-0">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
               <div className={`p-3 sm:p-4 rounded-2xl sm:rounded-3xl shrink-0 ${occurrence.status === 'Aberto' ? 'bg-red-100 text-red-600' : occurrence.status === 'Em Andamento' ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}`}><AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6" /></div>
               <div className="min-w-0"><h4 className="text-lg sm:text-xl md:text-2xl font-black uppercase tracking-tight leading-none">Ocorrência</h4><p className="text-[9px] sm:text-[10px] font-bold opacity-40 uppercase tracking-widest mt-1">{currentRole === 'PORTEIRO' ? 'Apenas visualização' : (currentRole === 'SINDICO' || currentRole === 'ADMIN' || currentRole === 'ADMINISTRADOR' || currentRole === 'ADMINISTRADORA' || currentRole === 'CABO_TURMA') ? 'Detalhes & Comunicação' : 'Detalhes'}</p></div>
            </div>
            <button onClick={onClose} className="p-2 sm:p-3 bg-zinc-100 rounded-xl sm:rounded-2xl hover:bg-zinc-200 shrink-0"><X className="w-4 h-4 sm:w-5 sm:h-5"/></button>
         </header>
         <div className="flex-1 overflow-y-auto space-y-4 sm:space-y-6 pr-2">
            <div className="p-4 sm:p-5 bg-zinc-50 rounded-[20px] sm:rounded-[24px] border border-black/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
               <div className="min-w-0"><p className="text-[9px] font-black uppercase opacity-30 mb-1">Unidade Afetada</p><p className="text-xs sm:text-sm font-bold uppercase wrap-break-word">{occurrence.unit} - {occurrence.residentName}</p></div>
               <div className="text-left sm:text-right"><p className="text-[9px] font-black uppercase opacity-30 mb-1">Data</p><p className="text-[10px] sm:text-xs font-bold uppercase opacity-60">{occurrence.date ? new Date(occurrence.date).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : occurrence.date}</p></div>
            </div>

            {/* Síndico/Admin podem alterar status; porteiro só visualiza */}
            {(currentRole === 'SINDICO' || currentRole === 'ADMIN' || currentRole === 'ADMINISTRADOR' || currentRole === 'ADMINISTRADORA' || currentRole === 'CABO_TURMA') && (
              <div>
            ...
            {/* Lista de mensagens (respostas do admin / comentários) — visível para todos, especialmente para o morador ver o que o admin respondeu */}
            {messages && messages.length > 0 && (
              <div className="space-y-3">
                <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest opacity-40 ml-2 mb-2 block">
                  Respostas e comentários
                </label>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 border border-zinc-100 rounded-[20px] sm:rounded-[24px] p-3 sm:p-4 bg-zinc-50">
                  {messages.map((msg: any) => {
                    const isFromStaff = ['SINDICO', 'PORTEIRO', 'ADMIN', 'ADMINISTRADOR', 'ADMINISTRADORA', 'CABO_TURMA'].includes(msg.senderRole);
                    const date = msg.timestamp ? new Date(msg.timestamp).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';
                    return (
                      <div
                        key={msg.id || msg.timestamp + msg.text}
                        className={`p-3 rounded-xl text-xs sm:text-sm ${isFromStaff ? 'bg-blue-50 border border-blue-100' : 'bg-zinc-100 border border-zinc-200'}`}
                      >
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span className="font-bold text-black/80">
                            {msg.senderName || (msg.senderRole === 'SINDICO' ? 'Síndico' : msg.senderRole === 'PORTEIRO' ? 'Porteiro' : msg.senderRole === 'ADMIN' || msg.senderRole === 'ADMINISTRADOR' || msg.senderRole === 'ADMINISTRADORA' ? 'Admin' : 'Morador')}
                          </span>
                          {date && <span className="text-[10px] font-medium opacity-50">{date}</span>}
                        </div>
                        <p className="text-zinc-800 whitespace-pre-wrap">{msg.text}</p>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Campo para mensagem: só síndico/admin podem responder; porteiro só visualiza */}
            {(currentRole === 'SINDICO' || currentRole === 'ADMIN' || currentRole === 'ADMINISTRADOR' || currentRole === 'ADMINISTRADORA' || currentRole === 'CABO_TURMA') && (
            <div className="flex gap-2">
                <input
                  type="text"
         ...
         {currentRole !== 'PORTEIRO' && (currentRole === 'SINDICO' || currentRole === 'MORADOR' || currentRole === 'ADMIN' || currentRole === 'ADMINISTRADOR' || currentRole === 'ADMINISTRADORA' || currentRole === 'CABO_TURMA') && (
           <div className="shrink-0 mt-6 pt-4 border-t border-zinc-100 flex flex-col sm:flex-row gap-3">
             ...
             {canResolve && (currentRole === 'SINDICO' || currentRole === 'ADMIN' || currentRole === 'ADMINISTRADOR' || currentRole === 'ADMINISTRADORA' || currentRole === 'CABO_TURMA') && occurrence.status === 'Aberto' && onResolve && (
               <button
                 onClick={() => {
                   onResolve(occurrence.id, occurrence);
                   setOccurrence({ ...occurrence, status: 'Resolvido' });
                 }}
                 className="flex-1 py-4 sm:py-5 md:py-6 bg-zinc-100 text-black rounded-[24px] sm:rounded-[32px] font-black uppercase text-[10px] sm:text-[11px] tracking-widest hover:bg-zinc-200 active:scale-95 transition-all shadow-lg flex items-center justify-center gap-2 sm:gap-3 border border-black/5"
               >
                 <Check className="w-4 h-4" /> <span className="whitespace-nowrap">Resolver</span>
               </button>
             )}
             {canDelete && occurrence.status === 'Resolvido' && onDelete && (
               <button
                 onClick={() => { onDelete(occurrence.id); onClose(); }}
                 className="px-6 py-4 sm:py-5 md:py-6 bg-red-500/10 text-red-600 border border-red-500/30 rounded-[24px] sm:rounded-[32px] font-black uppercase text-[10px] sm:text-[11px] tracking-widest hover:bg-red-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 shrink-0"
                 title="Excluir ocorrência (remove da lista)"
               >
                 <X className="w-4 h-4" /> <span className="whitespace-nowrap">Excluir</span>
               </button>
             )}
           </div>
         )}
    ...
    <div className="fixed inset-0 z-500 flex items-center justify-center p-4">
       ...
       <div className="relative w-full max-w-lg bg-white text-black rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-10 animate-in duration-300">
          <header className="flex justify-between items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
             ...
             <button onClick={onClose} className="p-2 sm:p-3 bg-zinc-100 rounded-xl sm:rounded-2xl hover:bg-zinc-200 transition-all shrink-0"><X className="w-4 h-4 sm:w-5 sm:h-5"/></button>
          </header>
          <div className="space-y-4 sm:space-y-5">
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                {/* Campo de senha - apenas para síndico e admin */}
                {['SINDICO', 'ADMIN', 'ADMINISTRADOR', 'ADMINISTRADORA'].includes(role) && data.id && (
    ...
    <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white text-black rounded-[32px] sm:rounded-[40px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-10 animate-in zoom-in duration-300">
         <header className="flex justify-between items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
           ...
           <button onClick={onClose} className="p-2 sm:p-3 bg-zinc-100 rounded-xl sm:rounded-2xl shrink-0">
         ...
         <div className="space-y-3 sm:space-y-4">
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-3 sm:py-4 bg-red-600 text-white rounded-xl sm:rounded-2xl font-black uppercase text-[9px] sm:text-[10px] shadow-xl mt-4 transition-all hover:bg-red-700 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
  ...
  return (
    <div className="fixed inset-0 z-600 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white text-black rounded-[32px] sm:rounded-[48px] shadow-2xl p-4 sm:p-6 md:p-8 lg:p-12 animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
         <header className="flex justify-between items-start gap-3 sm:gap-4 mb-6 sm:mb-8">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
              <div className="p-3 sm:p-4 rounded-2xl sm:rounded-3xl bg-green-100 text-green-600 shrink-0"><Bell className="w-5 h-5 sm:w-6 sm:h-6" /></div>
              <div className="min-w-0">
                <h4 className="text-lg sm:text-xl md:text-2xl font-black uppercase tracking-tight leading-none">Mural de Avisos</h4>
         ...
         <div className="space-y-4 sm:space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={handleFileChange} aria-hidden />
              <input ref={galleryInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileChange} aria-hidden />
              <div className="flex gap-2">
                <button type="button" onClick={() => cameraInputRef.current?.click()} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-dashed border-zinc-200 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:bg-zinc-50 transition-colors"><Camera className="w-4 h-4" /> Câmera</button>
                <button type="button" onClick={() => galleryInputRef.current?.click()} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-zinc-200 text-[10px] font-black uppercase tracking-widest text-zinc-600 hover:bg-zinc-50 transition-colors"><ImageIcon className="w-4 h-4" /> Galeria</button>
              </div>
              {imagePreview && (
