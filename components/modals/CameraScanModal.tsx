...
interface CameraScanModalProps {
  ...
  /** Modo inicial ao abrir: 'qr' (câmera/escaneamento) ou 'photo' (foto/galeria). */
  initialMode?: 'qr' | 'photo';
}
...
const CameraScanModal: React.FC<CameraScanModalProps> = ({
  ...
  const [mode, setMode] = useState<'qr' | 'photo'>(initialMode);
  ...
  // Ao abrir, aplicar modo inicial (câmera vs galeria).
  useEffect(() => {
  ...
  const showActivate = Boolean(
    isOpen && !stream && !capturedImage && status === 'idle' && !displayError
  );
  useEffect(() => {
  ...
  // Modal com câmera (mobile e desktop)
  return (
