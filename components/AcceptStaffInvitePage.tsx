...
const AcceptStaffInvitePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/staff-invite?token=${encodeURIComponent(t)}`)
      ...
      .then((data) => {
        if (data.error) {
          ...
          if (isLocalhost && !data.code && data.status === 404) {
            message = 'Serviço de convite indisponível. Em desenvolvimento, rode em outro terminal: npm run dev:api (ou npm run dev:all).';
          } else if (data.status === 500 && data.code === 'CONFIG_MISSING' && isLocalhost) {
            message = 'API sem configuração. No .env.local defina SUPABASE_SERVICE_ROLE_KEY e SUPABASE_URL (ou VITE_SUPABASE_URL) e reinicie o npm run dev:api.';
      ...
      .catch(() => {
        ...
        setError(isLocalhost
          ? 'Não foi possível validar o link. Rode em outro terminal: npm run dev:api (ou use npm run dev:all).'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/api/accept-staff-invite`, {
        method: 'POST',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#050505] p-4">
        <div className="max-w-md w-full backdrop-blur-3xl border border-white/10 rounded-[48px] p-8 shadow-2xl bg-white/[0.03]">
          ...
          <a
            href="/"
            className="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-white text-black font-bold text-sm uppercase hover:opacity-90 transition-opacity"
          >
  ...
  let roleLabel = 'Portaria';
  if (invite?.role === 'SINDICO') roleLabel = 'ADM';
  if (invite?.role === 'CABO_TURMA') roleLabel = 'Cabo de Turma';
