...
import { Pin, Crown, BadgeInfo, Check, CheckCheck, MessageSquareText, ChevronUp, X, SendHorizontal, RefreshCw, Trash2, Plus, Edit2, Phone } from 'lucide-react';
...
interface NoticesViewProps {
  ...
  /** Morador: dispensar/fechar o aviso após confirmar leitura */
...
const NoticesView: React.FC<NoticesViewProps> = ({
  ...
  const canManageNotices = ['PORTEIRO', 'SINDICO', 'ADMIN', 'ADMINISTRADOR', 'ADMINISTRADORA'].includes(role) && (onAddNotice != null || onEditNotice != null || onDeleteNotice != null);
  ...
  return (
    <div className="relative h-[calc(100vh-140px)] flex flex-col md:flex-row gap-6 animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
         ...
         <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-end mb-6 pb-6 border-b border-[var(--border-color)] flex-shrink-0">
            <div className="min-w-0">
               <h3 className="text-xl sm:text-2xl md:text-3xl font-black uppercase tracking-tighter truncate" style={{ color: 'var(--text-primary)' }}>Mural Digital</h3>
               <p className="text-[10px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] mt-1 truncate" style={{ color: 'var(--text-secondary)' }}>Avisos & Comunicados</p>
            </div>
            <div className="flex flex-wrap items-center gap-2 min-w-0">
               {canManageNotices && onAddNotice && (
                  <button
                     type="button"
                     onClick={onAddNotice}
                     className="px-3 py-2 sm:px-4 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all flex items-center gap-1.5 sm:gap-2 bg-green-600 text-white border-green-600 hover:bg-green-500 hover:border-green-500 shrink-0"
                  >
                     <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> <span className="whitespace-nowrap">Adicionar aviso</span>
                  </button>
               )}
               {['all', 'urgent', 'unread'].map(f => (
                  <button
                     ...
                     className={`px-3 py-2 sm:px-4 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all shrink-0 ${
                        noticeFilter === f 
               ...
               {filteredNotices.map((notice) => {
                  ...
                  return (
                     ...
                     >
                        ...
                        {/* Top-right: Pinned + Dispensar (após leitura) + Editar/Excluir (Portaria e Síndico) */}
                        <div className="absolute top-4 right-4 z-10 flex items-center gap-1.5">
                           ...
                           {notice.read && onDismissNotice && (
                              <button
                                 type="button"
                                 onClick={(e) => { e.stopPropagation(); onDismissNotice(notice.id); }}
      ...
      {/* COLUNA DIREITA (DESKTOP) / BOTTOM SHEET (MOBILE): CHAT "LINHA DIRETA" — Portaria, Síndico e Admin */}
      {['PORTEIRO', 'SINDICO', 'ADMIN', 'ADMINISTRADOR', 'ADMINISTRADORA'].includes(role) && (
      <div 
         className={`
