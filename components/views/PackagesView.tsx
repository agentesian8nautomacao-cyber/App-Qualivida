...
import { Search, Plus, Camera, Image as ImageIcon, Users, ChevronDown, ChevronUp, Trash2, Filter, Database } from 'lucide-react';
import { Package, Resident } from '../../types';
import { formatUnit } from '../../utils/unitFormatter';
import { isMobile } from '../../utils/deviceDetection';
import { exportPackagesToPDF } from '../../utils/exportPackages';
...
interface PackagesViewProps {
   ...
   */
  canRegister?: boolean;
  /** Exibe o botão Backup para portaria fazer backup das encomendas do turno. */
...
const PackagesView: React.FC<PackagesViewProps> = ({
  ...
  const handleBackupTurno = async () => {
    if (allPackages.length === 0) {
      alert('Nenhuma encomenda registrada para gerar o backup.');
      return;
    }
    setIsBackupLoading(true);
    try {
      await exportPackagesToPDF(allPackages, config.condominiumName || 'Qualivida Club Residence');
    } finally {
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            ...
            {showBackupButton && (
              <button
                type="button"
                onClick={handleBackupTurno}
                disabled={isBackupLoading || allPackages.length === 0}
                ...
                title="Backup das encomendas registradas no turno (PDF)"
              >
                <Database className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isBackupLoading ? 'animate-spin' : ''}`} />
                <span className="hidden sm:inline">{isBackupLoading ? 'Gerando...' : 'Backup'}</span>
              </button>
            )}
            {canRegister && (
              <button
