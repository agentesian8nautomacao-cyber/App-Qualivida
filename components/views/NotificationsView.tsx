...
import { Bell, Package, CheckCircle2, X, Eye, Clock, AlertCircle } from 'lucide-react';
import { Notification, Package as PackageType, Occurrence } from '../../types';
...
const NotificationsView: React.FC<NotificationsViewProps> = ({
  ...
  const handleViewOccurrence = (notification: Notification) => {
    if (notification.type !== 'occurrence' || !notification.related_id || !onViewOccurrence) return;
    const idStr = String(notification.related_id).trim();
    onViewOccurrence(idStr);
  };
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredNotifications.map(notification => (
            <div
              key={notification.id}
              role={notification.type === 'occurrence' && onViewOccurrence ? 'button' : undefined}
              tabIndex={notification.type === 'occurrence' && onViewOccurrence ? 0 : undefined}
              onClick={
                notification.type === 'occurrence' && onViewOccurrence
                  ? () => handleViewOccurrence(notification)
                  : undefined
              }
              onKeyDown={
                notification.type === 'occurrence' && onViewOccurrence
                  ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleViewOccurrence(notification); } }
                  : undefined
              }
              className={`premium-glass rounded-2xl p-5 border transition-all group relative ${
                ...
                notification.type === 'occurrence' && onViewOccurrence ? 'cursor-pointer' : ''
              }`}
            >
              {/* Botão X para deletar */}
              <button
                onClick={(e) => { e.stopPropagation(); handleDeleteClick(notification.id, e); }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                    ...
                    <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex-wrap relative z-10">
                      ...
                      {notification.type === 'occurrence' && notification.related_id && onViewOccurrence && (
                        <button
                          type="button"
                          onClick={(e) => {
                            e.preventDefault();
