...
import { chatWithConcierge } from '../../../services/sentinelaService';
...
interface ChatAssistantProps {
  ...
  onLiveCall?: () => void;
  onSettings?: () => void;
  userProfile: SentinelaUserProfile | null;
  occurrences?: OccurrenceItemSentinela[];
  onAddOccurrence?: (item: OccurrenceItemSentinela) => void;
  isFullScreen?: boolean;
  externalMessages?: SentinelaChatMessage[];
}

const ROLE_CONFIG: Record<
  string,
  {
    theme: { accentText: string; accentBorder: string; fabColor: string; userMessageBg: string };
    labels: { module: string; placeholder: string };
    quickActions: { text: string; label: string; icon: React.ComponentType<{ size?: number }>; color: string }[];
  }
> = {
  Síndico: {
    theme: {
    ...
    labels: { module: 'Módulo Síndico', placeholder: 'Digite sua solicitação administrativa...' },
    quickActions: [
  ...
  Porteiro: {
    ...
    labels: { module: 'Módulo Portaria', placeholder: 'Descreva a ocorrência...' },
    quickActions: [
  ...
  Morador: {
    theme: { accentText: 'text-blue-500', accentBorder: 'border-blue-500', fabColor: 'bg-blue-500', userMessageBg: 'bg-blue-500' },
...
const MessageBubble: React.FC<{ message: SentinelaChatMessage }> = ({ message }) => {
      ...
      <div className="flex justify-start animate-in slide-in-from-left fade-in duration-300 mb-6">
        <div className="flex items-end gap-2 max-w-[85%]">
          ...
          <div className="bg-[#1e293b] border border-blue-500/30 text-zinc-100 px-6 py-4 rounded-[2rem] rounded-bl-sm shadow-md">
            <div className="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1">{message.senderName ?? 'Morador'}</div>
  ...
  if (message.text.startsWith('[TASK]')) {
    try {
      const taskData = JSON.parse(message.text.replace('[TASK]', '')) as Task;
      return (
        <div className="flex justify-start animate-in slide-in-from-left fade-in duration-300 mb-6">
          <div className="w-[90%] bg-[#18181B] border border-white/10 p-5 rounded-[2rem] rounded-tl-none relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${taskData.priority === 'urgent' ? 'bg-red-500' : taskData.priority === 'high' ? 'bg-orange-500' : 'bg-blue-500'}`} />
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-white/5 rounded-lg text-[10px] text-zinc-400 uppercase font-bold border border-white/5">{taskData.category}</span>
              <span className="px-2 py-1 bg-white/5 rounded-lg text-[10px] text-zinc-400 uppercase font-bold border border-white/5">Prazo: {taskData.deadline}</span>
            </div>
          </div>
        </div>
      );
  ...
  const isLogConfirmation =
    message.text.includes('Evento registrado') ||
          ...
          if (block.type === 'header') {
            return (
              <div key={i} className="mb-3 mt-4 first:mt-0">
                <h3 className="font-serif text-lg font-bold text-white flex items-center gap-2">{block.content}</h3>
              </div>
            );
          }
          if (block.type === 'quote') {
              ...
              <div key={i} className="space-y-2 mb-3 pl-2 border-l border-zinc-800">
                {block.items.map((item, j) => (
                  <div key={j} className="flex gap-3 items-start">
                    <div className="mt-1.5 min-w-[6px] h-[6px] rounded-full bg-zinc-500" />
...
const ChatAssistant: React.FC<ChatAssistantProps> = ({
  ...
  const [localMessages, setLocalMessages] = useState<SentinelaChatMessage[]>([]);
  const [isTaskCreatorOpen, setIsTaskCreatorOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  const role: SentinelaRole = userProfile?.role ?? 'Porteiro';
  const config = ROLE_CONFIG[role] ?? ROLE_CONFIG.Porteiro;
  const isManager = role === 'Síndico';
  ...
  useEffect(() => {
    if (localMessages.length === 0 && userProfile) {
      const firstName = userProfile.name?.split(' ')[0] ?? 'Operador';
      const greeting = isManager
        ? `Olá, Síndico(a) ${firstName}.\n\nSou ${assistantName ?? 'Conselheiro'}, seu assistente administrativo. Como posso ajudar na gestão hoje?`
        : `Olá, ${firstName}.\n\nSou ${assistantName ?? 'Sentinela'}, o assistente de portaria. Posso registrar visitantes, encomendas ou ocorrências.`;
      setLocalMessages([{ id: '1', role: 'model', text: greeting, timestamp: Date.now() }]);
    }
  }, [userProfile?.role, userProfile?.name, isManager, assistantName]);
  ...
  const handleReset = () => {
    if (confirm('Limpar histórico de conversa?')) setLocalMessages([]);
  };
  ...
  const handleTaskCreated = (task: Task) => {
    ...
    setTimeout(() => {
      setLocalMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: 'model', text: `[TASK]${JSON.stringify(task)}`, timestamp: Date.now() },
      ]);
    }, 500);
  };
  ...
  const handleSend = async () => {
    ...
    const userMsg: SentinelaChatMessage = {
      id: Date.now().toString(),
      role: 'user',
      text: input,
      timestamp: Date.now(),
    ...
    try {
      ...
      setLocalMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: 'model', text: 'Sem sinal da central. Tente novamente.', timestamp: Date.now() },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const containerClass = isFullScreen
    ? 'fixed inset-0 bg-[#09090b] z-50 flex flex-col'
    : 'flex flex-col h-full min-h-0 bg-[#09090b] rounded-[2rem] border border-white/5 overflow-hidden';
  ...
  return (
    <div className={containerClass}>
      <TaskCreator
        ...
        userRole={userProfile?.role ?? 'Porteiro'}
      ...
      <div className={`bg-[#09090b]/80 backdrop-blur-md px-6 py-4 flex justify-between items-center sticky top-0 z-10 border-b ${config.theme.accentBorder}`}>
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full border overflow-hidden flex items-center justify-center bg-zinc-900 ${isManager ? 'border-amber-500 text-amber-500' : 'border-zinc-700 text-white'}`}>
            {assistantAvatar ? <img src={assistantAvatar} className="w-full h-full object-cover" alt="AI" /> : <Bot size={20} />}
          </div>
          <div>
            <h3 className={`font-serif text-lg tracking-wide leading-none ${config.theme.accentText}`}>{assistantName ?? 'Sentinela'}</h3>
        ...
        <div className="flex gap-4 items-center">
          ...
          <button type="button" onClick={handleReset} className="text-zinc-500 hover:text-white transition-colors">
            <RefreshCcw size={18} />
          </button>
          {onSettings && (
            <button type="button" onClick={onSettings} className="text-zinc-500 hover:text-white transition-colors">
              <Settings2 size={18} />
            </button>
          )}
          {onClose && (
            <button type="button" onClick={onClose} className="text-zinc-500 hover:text-white transition-colors p-2" title="Fechar">
              ✕
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-[#09090b] min-h-0">
        {allMessages.map((msg) => (
        ...
        <div className="flex items-end gap-3 max-w-2xl mx-auto">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0 ${
              input.trim() ? config.theme.fabColor : 'bg-zinc-800 text-zinc-500'
            }`}
          >
            <Send size={20} />
          </button>
