...
import { X, Calendar, Wrench, Shield, Package, Trash2, ArrowRight, Flag, Layers, Briefcase } from 'lucide-react';
import type { SentinelaRole, TaskPriority, TaskCategory, Task } from '../../../types/sentinela';

interface TaskCreatorProps {
  isOpen: boolean;
  onClose: () => void;
  userRole: SentinelaRole;
  onSave: (task: Task) => void;
}

const THEMES: Record<string, { accentText: string; bgAccent: string; headerLabel: string; headerTitle: string; buttonLabel: string }> = {
  Síndico: {
...
const TaskCreator: React.FC<TaskCreatorProps> = ({ isOpen, onClose, userRole, onSave }) => {
  ...
  const theme = THEMES[userRole] ?? THEMES.Porteiro;
  ...
  const categories: { id: TaskCategory; label: string; icon: React.ComponentType<{ size?: number }> }[] = [
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center">
      ...
      <div className="relative w-full max-w-lg bg-[#18181b] rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-500 border border-white/5 overflow-hidden">
        <div className={`absolute top-0 left-0 right-0 h-1.5 ${theme.bgAccent} opacity-50`} />
        <div className="flex justify-between items-start mb-6">
          <div>
            <span className={`text-[10px] font-bold tracking-[0.2em] uppercase ${theme.accentText} opacity-80 mb-1 block`}>
              {theme.headerLabel}
            </span>
            <h2 className="text-white font-serif text-2xl">{theme.headerTitle}</h2>
          </div>
          <button onClick={onClose} className="p-2 bg-white/5 rounded-full text-zinc-400 hover:text-white hover:bg-white/10 transition-colors">
            <X size={20} />
          </button>
        </div>
        <div className="space-y-6">
          <input
            ...
            className="w-full bg-transparent text-3xl font-serif text-white placeholder:text-zinc-700 outline-none border-none p-0"
            autoFocus
          />
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Adicione detalhes, local ou instruções..."
                ...
                {priorities.map((p) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full flex items-center gap-2 transition-all border ${
                      priority === p.id ? 'bg-white/10 border-white/20 text-white' : 'bg-transparent border-white/5 text-zinc-600 hover:text-zinc-400'
                    }`}
                  >
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl flex items-center gap-2 transition-all border flex-shrink-0 ${
                        category === c.id ? 'bg-white/10 border-white/20 text-white' : 'bg-transparent border-white/5 text-zinc-600 hover:text-zinc-400'
                      }`}
                    >
                      <Icon size={14} />
                ...
                {deadlines.map((d) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border whitespace-nowrap ${
                      deadline === d ? 'bg-zinc-800 border-zinc-600 text-zinc-200' : 'bg-transparent border-zinc-800 text-zinc-600 hover:border-zinc-700'
                    }`}
                  >
                    {d}
                  </button>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex justify-end">
          <button
            onClick={handleSave}
            disabled={!title.trim()}
            className={`flex items-center gap-2 px-8 py-4 rounded-2xl font-bold text-black transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg ${theme.bgAccent}`}
          >
            {theme.buttonLabel}
            <ArrowRight size={20} />
          </button>
