...
import { Boleto, BoletoType, Resident, UserRole } from '../../types';
...
const BoletosView: React.FC<BoletosViewProps> = ({
  ...
  // (Removido) Monitoramento de rota via window.location/MutationObserver:
  // a navegação do app é por abas (estado), não por URL.
        ...
        {sortedBoletos.length === 0 ? (
          ...
          <div className="grid gap-4">
            {sortedBoletos.map((boleto) => (
              ...
              >
                {/* Ações rápidas (morador): baixar + excluir */}
                <div className="absolute top-3 right-3 z-30 flex items-center gap-2">
                  {(boleto.pdf_original_path || boleto.pdfUrl) && (
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-[var(--glass-bg)] border border-[var(--border-color)] hover:bg-[var(--border-color)] transition-all opacity-90"
