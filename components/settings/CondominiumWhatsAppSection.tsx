...
const CondominiumWhatsAppSection: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (!normalization.isValid || !normalization.normalized) {
      toast.error(`Número de WhatsApp inválido: ${normalization.error || 'Formato incorreto'}. Verifique o número e tente novamente. Exemplo: 5511999999999 (código do país + DDD + número)`);
      return;
    }
    ...
    <div className="premium-glass rounded-[32px] p-4 sm:p-6 md:p-8 min-w-0 overflow-hidden">
      <div className="flex flex-wrap items-center gap-3 mb-6 min-w-0">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-[var(--glass-bg)] flex items-center justify-center border border-[var(--border-color)] shrink-0">
          <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6 text-[var(--text-primary)]" />
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="text-lg sm:text-xl font-black uppercase tracking-tight">WhatsApp do Condomínio</h3>
